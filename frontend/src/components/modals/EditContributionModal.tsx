...
export function EditContributionModal({ open, onOpenChange, contribution, onSuccess }: EditContributionModalProps) {
  ...
  useEffect(() => {
    if (contribution) {
      setFormData({
        ...
        fund: contribution.fund_id,
        amount: contribution.amount.toString(),
        channel: contribution.payment_method || (contribution.channel === "online" ? "Mobile Money" : "Cash"),
        dateReceived: new Date(contribution.date_received),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!account?.account_id || !contribution) {
      toast({
        title: "Error",
        description: "Account or contribution not found",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const response = await contributionApi.update(contribution.contribution_id, {
        member_id: formData.member,
        amount: amount,
        channel: channel,
        payment_method: formData.channel,
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to update contribution";
      toast({
        title: "Error",
                          ...
                          <CommandGroup>
                            {filteredFunds.map((fund) => (
                              <CommandItem
                                ...
                                onSelect={() => {
                                  setFormData({ ...formData, fund: fund.fund_id });
                                  setFundOpen(false);
                                }}
                              >
                                <Check
